...
export class SubscribeController {
  ...
  async getLatestUrl(@Res() res: any) {
    ...
    const fetchedAtHuman = fetchedAt
      ? new Date(fetchedAt).toLocaleString('zh-CN', { hour12: false, timeZone: tz })
      : '无';
    const userinfoRaw = (meta?.headers && (meta.headers['subscription-userinfo'] || meta.headers['Subscription-Userinfo'])) || meta?.subscriptionUserinfo || '';
    let uploadHuman = '未知';
    let downloadHuman = '未知';
    let totalHuman = '未知';
    let expireHuman = '无';
    try {
      ...
      String(userinfoRaw)
        ...
        .forEach((pair) => {
          ...
          if (i > 0) kv[pair.slice(0, i)] = Number(pair.slice(i + 1));
        });
      ...
      uploadHuman = fmt(kv.upload || 0);
      downloadHuman = fmt(kv.download || 0);
      totalHuman = fmt(kv.total || 0);
      const ts = kv.expire || 0;
      if (isFinite(ts) && ts > 0) {
        expireHuman = new Date(ts * 1000).toLocaleString('zh-CN', {
          hour12: false,
          timeZone: tz,
        });
      }
    ...
    const url = meta?.url || '';
    const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>订阅状态</title><style>body{margin:0;padding:24px;background:#f7f7f8;color:#111;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Oxygen,Ubuntu,Cantarell,Helvetica Neue,Arial,sans-serif}main{max-width:720px;margin:0 auto}h1{font-size:20px;margin:0 0 16px}section{background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:16px;margin-bottom:16px;box-shadow:0 1px 2px rgba(0,0,0,0.04)}.row{display:flex;justify-content:space-between;align-items:center;margin:8px 0}.label{color:#666}.value{font-weight:600}.badge{display:inline-block;padding:2px 8px;border-radius:999px;font-size:12px;font-weight:600}.ok{background:#e6ffed;color:#047857;border:1px solid #a7f3d0}.warn{background:#fff7ed;color:#b45309;border:1px solid #fed7aa}.counts{display:flex;gap:12px;margin-top:8px}.count{background:#f3f4f6;border:1px solid #e5e7eb;border-radius:8px;padding:8px 12px}.links a{display:inline-block;margin-right:12px;color:#2563eb;text-decoration:none}.links a:hover{text-decoration:underline}.btn{display:inline-block;margin-top:12px;padding:8px 12px;border:1px solid #e5e7eb;border-radius:8px;background:#2563eb;color:#fff;font-weight:600;cursor:pointer}.btn:disabled{opacity:.6;cursor:not-allowed}</style></head><body><main><h1>订阅状态</h1><section><div class="row"><div class="label">状态</div><div class="value"><span class="badge ${hasData ? 'ok' : 'warn'}">${hasData ? '已获取数据' : '暂无数据'}</span></div></div><div class="row"><div class="label">最近抓取时间</div><div class="value">${fetchedAtHuman}</div></div><div class="row"><div class="label">订阅地址</div><div class="value">${url ? `<a href="${url}" target="_blank" rel="noopener noreferrer">${url}</a>` : '无'}</div></div><div class="counts"><div class="count">节点 ${counts.proxies || 0}</div><div class="count">分组 ${counts.groups || 0}</div><div class="count">规则 ${counts.rules || 0}</div></div><div class="counts"><div class="count">上传 ${uploadHuman}</div><div class="count">下载 ${downloadHuman}</div><div class="count">总量 ${totalHuman}</div><div class="count">过期 ${expireHuman}</div></div><button id="refreshBtn" class="btn">立即刷新</button></section><section class="links"><a href="/api/subscribe/clash" target="_blank">查看 Clash YAML</a><a href="/api/subscribe/sample" target="_blank">示例 YAML</a><a href="/api/subscribe/shadowrocket" target="_blank">Shadowrocket 链接</a></section></main><script>(function(){var b=document.getElementById('refreshBtn');if(!b)return;b.addEventListener('click',async function(){var k=window.prompt('请输入API_KEY')||'';if(!k)return;b.disabled=true;b.textContent='刷新中…';try{var r=await fetch('/api/subscribe/refresh',{method:'POST',headers:{'x-api-key':k}});if(r.ok){location.reload();}else{var t=await r.text();alert(t||('刷新失败 '+r.status));}}catch(e){alert('请求异常');}finally{b.disabled=false;b.textContent='立即刷新';}});})();</script></body></html>`;
